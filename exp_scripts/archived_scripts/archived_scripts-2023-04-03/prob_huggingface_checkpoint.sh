...
do 
    ...
    for dataset in ${TEST_DATASET}; do {
        ...
        if [ ${ENABLE_EXTRACT} == 'true' ]; then
            ...
            bash exp_scripts/extract_features_by_dataset.sh ${HF_CHECKPOINT_DIR} ${MODEL_TYPE} ${OUTPUT_FEAT_ROOT} ${dataset} ${TARGET_SAMPLE_RATE} ${PROCESSOR_NORMALIZE} ${ACCELERATOR}
        fi
        ...
        if [ ${ENABLE_PROBE} == 'true' ]; then
            ...
            bash exp_scripts/probe_by_dataset.sh ${HF_CHECKPOINT_DIR} ${MODEL_TYPE} ${OUTPUT_FEAT_ROOT} ${dataset} ${MODEL_SETTING} ${ACCELERATOR} ${ACC_PRECISION}
        fi
    }
    done
done
